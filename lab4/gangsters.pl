...
needed(T,H,N):-  gangstersNeeded(T,L), nth1(H,L,N). % nth1( ?Index, ?List, ?Elem ):
...
% does(G,T,H) means:  "gangster G does task T at hour H"     (MANDATORY)
satVariable( does(G,T,H) ):- gangster(G), task(T), hour(H).
%works-G-H means: "gangster G works in some task at hour H"
satVariable( none(G,T,H) ):- gangster(G), task(T), hour(H).
...
writeClauses(K):-
    eachHourEachGangsterAtMostOneTask, %%en una hora un gangster solo puede hacer una tarea a la vez
    noMoreHoursThan(K), %%ningun gangster puede trabajar mas de k horas consecutivas
    allTasksDone, %q se hagan todas las tasks
    noDiffConsec, %1 gansgetr, no puede hacer dos tasks diferentes consecutivamente
    gangNotAvailable, %respetar las horas en que los gangsters no estan libres
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachHourEachGangsterAtMostOneTask:- gangster(G), hour(H), findall( does(G, T, H), task(T), List), atMost(1, List), fail.
eachHourEachGangsterAtMostOneTask.

% list of Size S starting at N
%listOf(S, N, R) :- M is S+N-1, findall(X, between(N, M, X), R).

noMoreHoursThan(K):- gangster(G), task(T), hour(H1), hour(H2), H2 is H1 + K,
    findall(does(G, T, H), between(H1, H2, H), Lits), negateAll(Lits, NLits), atLeast(1, NLits), fail.
noMoreHoursThan(_).

allTasksDone:- needed(T,H,N), findall(does(G,T,H), gangster(G), List), exactly(N, List), fail.
allTasksDone.

noDiffConsec:- available(G, H1), available(G, H2), H2 is H1+1, task(T1), task(T2), T1 \= T2,
