...
needed(T,H,N):-  gangstersNeeded(T,L), nth1(H,L,N). %% for each task and hour the number of gangsters needed
...
noDifferentTasksInConsecutiveHours:-
  % for each hour H1, hour H2, Gangster G, task T1, task T2
...
eachGangsterAtMostKConsecutiveHours(K):-
  ...
  between(1, E, N),
  listOf(KK, N, L),
  findall(does-G-T-H, member(H, L), Lits),
  expressAnd(aux-G-T-N, Lits),
  writeClause([\+aux-G-T-N]),
  fail.
...
main:-  symbolicOutput(1), !, writeClauses(72), halt.   % print the clauses in symbolic form and halt
main:-
