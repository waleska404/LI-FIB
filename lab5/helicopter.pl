...
main:- N=8, retractall(bestRouteSoFar(_,_)),  assertz(bestRouteSoFar(100000,[])),  % "infinite" distance
...
% No passengers to pick up, no passengers left in helicopter -->  return to base (he arribat a una fulla)
heli( [], AccumulatedDistance, [CurrentPoint|RouteSoFar], [] ):- 
helicopterBasePoint(BASE), distanceBetweenTwoPoints(CurrentPoint, BASE, HASTABASE), AD is AccumulatedDistance+HASTABASE,
storeRouteIfBetter(AD, [BASE, CurrentPoint|RouteSoFar]),  fail.
...
length(DestinationsOfPassengersInHelicopter,L),member(L,[0,1]),      % write(length(DestinationsOfPassengersInHelicopter)),
select(Passenger, PassengersToPickUp, RemainingPassengers), nth1(2, Passenger, DEST), nth1(1, Passenger, ORG),
distanceBetweenTwoPoints(CurrentPoint, ORG, PLUS), AccumulatedDistance1 is AccumulatedDistance+PLUS,
heli(RemainingPassengers, AccumulatedDistance1, [ORG, CurrentPoint|RouteSoFar], [DEST|DestinationsOfPassengersInHelicopter] ).

% Drop off one of the passengers in the helicopter:
heli( PassengersToPickUp, AccumulatedDistance, [CurrentPoint|RouteSoFar], DestinationsOfPassengersInHelicopter ):-
length(DestinationsOfPassengersInHelicopter,L),member(L,[1,2]), 
select(Destination, DestinationsOfPassengersInHelicopter, RemainingDestinations),
distanceBetweenTwoPoints(CurrentPoint, Destination, PLUS), AccumulatedDistance1 is AccumulatedDistance+PLUS,
heli(PassengersToPickUp, AccumulatedDistance1, [Destination, CurrentPoint|RouteSoFar], RemainingDestinations).
