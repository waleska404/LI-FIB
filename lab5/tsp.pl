...
tsp( [], AccumulatedKm, RouteSoFar ):- storeRouteIfBetter(AccumulatedKm,RouteSoFar), fail. %fail cuando he arribat a una fulla
tsp(  _, AccumulatedKm, _          ):- bestRouteSoFar(Km,_), AccumulatedKm >= Km, !, fail. %todavia no he visitado todas las cidudades pero el camino que llevo recorrido ya tiene mas coste que el mejor camino que he encontrado hasta ahora
